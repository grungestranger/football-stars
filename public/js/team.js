(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["/js/team"],{
...
$(function () {
  ...
  playersPositions(); // Select Settings
    ...
    stop: function stop(e, ui) {
      ...
      $('.player:visible').each(function () {
        ...
        $.each(roleAreas, function (k, v) {
          if (pos.x >= v.x[0] && pos.x <= v.x[1] && pos.y >= v.y[0] && pos.y <= v.y[1]) {
            if (!isset(temp[k])) {
              temp[k] = [];
            }
      ...
      var keys = []; // if you trust the search order of object properties,
  ...
  var fields = ['position', 'reserveIndex']; // replace rows
  ...
  $('#createSchema').click(function () {
    $.post('/team/create-schema', $('#schemaForm').serialize(), function (data) {
      ...
      $('#schemaForm [name="schema[id]"]').append('<option value="' + data.schema.id + '">' + hsc(data.schema.name) + '</option>').val(data.schema.id);
  ...
  $('#field').mousemove(function (e) {
    ...
    $('.role-area').each(function () {
      ...
      if (e.pageX - fieldOffset.left - margin >= coords.x[0] * fieldCoef && e.pageX - fieldOffset.left - margin <= coords.x[1] * fieldCoef && e.pageY - fieldOffset.top - margin <= (fieldHeight - coords.y[0]) * fieldCoef && e.pageY - fieldOffset.top - margin >= (fieldHeight - coords.y[1]) * fieldCoef) {
        $(this).addClass('hover');
        return false;
      }
    });
...
module.exports = __webpack_require__(/*! /home/vagrant/code/football-stars/resources/js/team.js */"./resources/js/team.js");
